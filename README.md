...
## Project Summary
Our Project was to design a mechanical system deemed appropriate by Dr. Dvorak and use the semester to successfully construct it while using/learning the Scrum model. The project we chose was a scaled down automated greenhouse that had a certain number of features that would allow for plant growth in a certain set way. The Automated Greenhouse’s primary features include an LCD displaying Temperature and Humidity, A cooling system using a fan motor, and a soil moisture sensor that regulates a submersion pump for water the plant through a tube. The Greenhouse is capable of holding a medium sized pot and a small storage vessel for water that the submersion pump can use to water the soil as needed. On the sides of the Greenhouse are a Mega Arduino controller and two breadboards that run the Master code stored on the Arduino so long has its two power inputs are connected to an outlet. The Greenhouse itself is composed of polycarbonate walls and roofing to provide insulation while its construction leaves some access to outside air for increased ventilation.  

Overall, the resulting Greenhouse was a success with all features included in the backlog being created as well as fully functioning as intended. The LCD does display both Temperature and Humidity 24/7 regardless of if both the Fan/Watering system is on or only one. The Fan checks the Temperature every 3 minutes and if the targeted temperature is reached will turn on then follow suit when turn off once the temperature is below the target value. Similarly, the submersion pump will at regular intervals check the soil moisture level once it is below the target value provide water to the soil once then repeat till the soil moisture level is above the target value. With all this successfully working, the Greenhouse can maintain a set amount of plants, provided they fit, and does model a larger scale creation of an Automated Greenhouse. 
## Design Description
The automated greenhouse measures 13 by 17 inches, allowing ample space to fit multiple plants while growing. It is composed of polycarbonate sheets and supported in all corners with wood framing. The standard wall heaight is 15 inches, and reaches 24 inches at the peak. Structural components, and sensors, are connected using hot glue as an adhesive. Two sensors are housed within the greenhouse: one in the soil to sense moisture, and one mounted on the side of the greenhouse to detect both temperature and humidity.  The sensors are connected to pins as shown in the schematic below.  The LCD displays the temperature and humidity percentage inside of the greenhouse.  If the soil moisture sensor detects below a certain value, the water pump turns on and waters the plants.  If the temperature/humidity sensor detects a temperature level too high for the plants, a DC motor fan is turned on to circulate cooler, fresh air in to the greenhouse. 
### Circuit Schematic
![image](https://user-images.githubusercontent.com/102317706/165352077-04833366-79a6-4dd3-8340-3d751659f54e.png)

### CAD Drawings
#### Structure with Electrical Components
...
## Testing
In order to test our completed design, we placed a curly wurly corkscrew rush plant into the greenhouse that had not been watered in several days.  After inserting the soil moisture sensor into the soil of the pot, we plugged our Arduino in to a 12V power source.  Since we were indoors in a lab with the structure, we had to heat up the temperature sensor inorganically in order to make the fan turn on.  To do this, we used a cigarette lighter to heat the sensor up until it was 80˚F.  In order to test the soil moisture sensor multiple times, we ran the ¼-inch airplane tubing from the water source back into the water source.  We did this so there would not be moisture added to our soil during testing that would disrupt future tests, but we were still able to ensure the pump was working.  We were able to visually test our LCD by comparing the display to a digital thermometer.   
## Testing Results
While we were constructing our final product, we performed preliminary testing.  During this testing, we encountered an issue with our LCD and water pump drawing too much power for the 5V output from the circuit, before we even added any other passive elements to our circuit.  As an attempt to remedy this, we added a transistor to the circuit.  Even with a transistor, while the pump was running, the LCD displayed strange characters instead of the temperature and humidity values.  We noticed this was happening as soon as the pump turned on, so we were able to deduce that it was still a power issue.  We further remedied this before adding any other elements to our circuit by trading the battery pack for a 12V wall outlet power supply, and connecting the water pump and the DC motor to one output pin on the Arduino, and all of our other circuit elements to the other output pin on the Arduino.  

![image](https://user-images.githubusercontent.com/102317706/165510901-e770242a-f471-4625-a275-a331a53dbde2.png)

During our testing, the bold numbers above are where we put our finger over the temperature/humidity sensor to raise the temperature it would read, thereby being able to verify that the fan turned on since we were inside a temperature-controlled building. Because the temperature of the sensor could not physically drop back down to the temperature of the room immediately, we observed a slow lowering of the temperature back down to the room temperature. Logically, we inferred that the temperature reading aspect of the sensor was working as well as the humidity sensor. For the soil moisture sensor, we removed sensor from the soil so that it would be below the 350-set limit for the soil to be considered dry at which time the submersion pump push water of a pipe into the soil at the 0-reading measure. The LCD displayed accurate readings of both temperature and humidity and remained normally for extended periods of time with both the fan on and off.  

The limitations of the devices are that one, since it is a scaled model only a single pot can be added into the system and the roof restricts plant height. Another is that the system has no way to electrically heat the greenhouse relying solely on the plant and insulation of the greenhouse limiting where the greenhouse can be used since a much colder climate would hinder any plant needing a warmer environment to grow. Finally, the system has no way to simulate sunlight and thus the greenhouse would need to be placed in a well-lit area for it to function properly. From this, the most effective way for this design is as a representation a scaled-up version capable of tending to many for plants than a few with our current design. However, the model still functions has a greenhouse and as such can still be used for that purpose.  
